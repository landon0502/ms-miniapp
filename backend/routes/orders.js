...
router.get('/', async (req, res) => {
  try {
    ...
    console.log('分页参数:', { page, pageSize, status, offset });
 ...
 *   post: 
 *     summary: 创建订单
 *     description: 创建新订单，包括订单基本信息和商品信息
 *     tags: [订单管理]
 *     requestBody:
 *       required: true
 *       content:
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { user_id, items, total_price, actual_price, departure_time, sub_order_no, order_time } = req.body;
    ...
    try {
      ...
      res.json({
        ...
        code: 200,
        message: '创建订单成功',
        data: { id: orderId, order_no, sub_order_no: generated_sub_order_no, user_id, total_price, actual_price, status: 'pending', departure_time, order_time, shipping_store: '海南电商离岛免税' }
      });
    } catch (orderError) {
 ...
 *   put: 
 *     summary: 更新订单
 *     description: 更新指定订单的信息，包括基本信息和商品信息
 *     tags: [订单管理]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 *   get: 
 *     summary: 获取订单详情
 *     description: 根据订单 ID 获取订单详情，包括订单基本信息和商品信息
 *     tags: [订单管理]
 *     parameters:
 *       - in: path
 *         name: id
...
router.get('/:id', async (req, res) => {
  try {
    console.log('开始获取订单详情，订单ID:', req.params.id);
    const pool = getPool();
 ...
 *   delete: 
 *     summary: 删除订单
 *     description: 根据订单 ID 删除订单，级联删除相关的订单项
 *     tags: [订单管理]
 *     parameters:
 *       - in: path
 *         name: id
...
router.delete('/:id', async (req, res) => {
  try {
    console.log('开始删除订单，订单ID:', req.params.id);
    const pool = getPool();
    const connection = await pool.getConnection();
