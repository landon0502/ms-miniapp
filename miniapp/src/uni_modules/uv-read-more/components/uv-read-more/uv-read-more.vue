<template>
	<view class="uv-read-more">
		<view
		    ...
		    :style="{
				height: isLongContent && openStatus === 'close' ? $uv.addUnit(showHeight) : $uv.addUnit(contentHeight,'px'),
		...
		>
			<slot name="toggle">
				...
				>
					<uv-text
					    :text="openStatus === 'close' ? closeText : openText"
					    :color="color"
					    :size="fontSize"
					    :lineHeight="fontSize"
					    margin="0 5px 0 0"
					...
					<view class="uv-read-more__toggle__icon">
						<uv-icon
						    :color="color"
						    :size="fontSize + 2"
						    :name="openStatus === 'close' ? 'arrow-down' : 'arrow-up'"
						></uv-icon>
					</view>
				</view>
			</slot>
	...
	export default {
		...
		data() {
			return {
				...
				openStatus: 'close', // 当前隐藏与显示的状态，close-收起状态，open-展开状态
				elId: '', // 生成唯一class
		...
		computed: {
			// 展开后无需阴影，收起时才需要阴影样式
			innerShadowStyle() {
				if (this.openStatus === 'open') return {}
		...
		methods: {
			...
			// 展开或者收起
			toggleReadMore() {
				this.openStatus = this.openStatus === 'close' ? 'open' : 'close'
				// 如果toggle为false，隐藏"收起"部分的内容
				if (this.toggle == false) this.isLongContent = false
				// 发出打开或者收齐的事件
				this.$emit(this.openStatus, this.name)
			}
		}
	}
</script>
