	...
	export default {
		...
		watch: {
			...
			value(newVal) {
				if (newVal !== this.currentValue) {
					this.currentValue = this.dynamicRestrictionsValue ? this.format(this.value) : this.value
				}
			},
			modelValue(newVal) {
				if (newVal !== this.currentValue) {
					this.currentValue = this.dynamicRestrictionsValue ? this.format(this.modelValue) : this.modelValue
				}
			}
		},
		computed: {
			...
			// 用于监听多个值发生变化
			watchChange() {
				if(this.dynamicRestrictionsValue){
		...
		methods: {
			...
			onBlur(event) {
				...
				if(this.inputBlurUpdateValue && this.isInputed){
					if(value === ''){
			...
			// 输入框值发生变化
			onInput(e, allowChange = !this.inputBlurUpdateValue) {
				 const {
					value = ''
				} = e.detail || {}
				this.isInputed = true
				// 为空返回
				if (value === '' || !allowChange) return
