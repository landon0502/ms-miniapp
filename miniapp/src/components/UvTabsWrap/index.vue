		...
		<view class="uv-tabs__wrapper">
			...
			<view class="uv-tabs__wrapper__scroll-view-wrapper">
				<scroll-view
					...
					class="uv-tabs__wrapper__scroll-view hibe-scrollbar"
					...
					>
						<view
							...
							:class="[
								...
								item.disabled && 'uv-tabs__wrapper__nav__item--disabled'
							]"
						>
							<text
...
import uvBadgeProps from '../Badge/props.js'
// #ifdef APP-NVUE
			...
			return (index) => {
				...
				const customeStyle =
					index === this.innerCurrent
						? this.$uv.addStyle(this.activeStyle)
						: this.$uv.addStyle(this.inactiveStyle)
		...
		getAllItemRect() {
			return new Promise((resolve) => {
				const promiseAllArr = this.list.map((item, index) =>
					this.queryRect(`uv-tabs__wrapper__nav__item-${index}`, true)
				)
