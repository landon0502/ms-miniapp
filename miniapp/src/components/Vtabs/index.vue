<template>
	<view class="uv-vtabs" :style="[vtabsStyle]">
		<scroll-view
			class="uv-vtabs__bar hide-scroll-bar"
			ref="uv-vtabs__bar"
			:style="[getBarStyle]"
		...
		<scroll-view
			class="uv-vtabs__content hide-scroll-bar"
			...
			v-if="chain && contentScrollable && showContent"
			enhanced
		>
			<slot />
		</scroll-view>
		<scroll-view
			v-else-if="contentScrollable && showContent"
			class="uv-vtabs__content hide-scroll-bar"
			:style="[getContentStyle, $uv.addStyle(contentStyle)]"
			:scroll-y="true"
			:scroll-x="scrollX"
		...
		<view
			...
			class="uv-vtabs__content no-scrollbar"
			:style="[getContentStyle, $uv.addStyle(contentStyle)]"
		>
			<slot />
		</view>
 ...
 * @property {Boolean}  contentScrollable 右侧内容区域是否允许滚动 （默认 true ）
...
export default {
	...
	mounted() {
		this.$nextTick(() => {
			let index = this.list?.findIndex?.((item) => item[this.keyValue] === this.modelValue)
			this.init(index)
		})
	},
	data() {
	...
	computed: {
			...
			return (index) => {
				...
				const customeStyle =
					index === this.activeIndex
						? this.$uv.addStyle(this.barItemActiveStyle)
						: this.$uv.addStyle(barItemInitStyle)
		...
		textStyle() {
			return (index) => {
				let style = { ...addStyle(this.barItemTextStyle) }
				if (index === this.activeIndex) {
					style = this.$uv.deepMerge(style, addStyle(this.barItemTextActiveStyle))
				}
				style.width = this.$uv.getPx(this.barWidth, true)
				return style
			}
	...
	watch: {
		modelValue(newVal) {
			if (!this.touching) {
				let index = this.list.findIndex((item) => item[this.keyValue] === newVal)
				this.$nextTick(() => {
					this.init(index < 0 ? 0 : index)
		...
		activeIndex(newVal) {
			if (!this.chain) {
				// 解决：非联动，内容过多的情况，滚动一段距离，再切换未滚动到顶部的BUG
				this.contentScrollTop2 = 0 - Math.random() * 4 - 4
			}
			let value = this.list[newVal]?.[this.keyValue]
			this.$emit('update:modelValue', value)
			this.$emit('change', { index: newVal, value })
		}
	},
	methods: {
		init(index) {
		...
		// 内容滚动到底部触发
		scrolltolower() {
			let key = this.list[this.activeIndex]?.[this.keyValue]
			this.$emit('scrolltolower', { index: this.activeIndex, value: key })
		},
		async resize() {
		...
		setBarScrollTop() {
			...
			const offsetTop = this.list.slice(0, this.activeIndex).reduce((total, item) => {
		...
		async clickHandler(currentIndex) {
			...
			if (this.chain) {
				...
				this.contentScrollTop =
					this.children[currentIndex].top - this.$uv.getPx(this.hdHeight) - Math.random() * 4 - 4
				// #ifndef MP-WEIXIN
		...
		getAllItemRect() {
			return new Promise((resolve) => {
				const promiseAllArr = this.list.map((item, index) =>
					this.queryRect(`uv-vtabs__bar-item--${index}`, true)
				)
